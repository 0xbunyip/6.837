...
Material::Material(const Vector3f &d_color, const Vector3f &s_color, float s,
                   float r)
    : diffuseColor(d_color), refractionIndex(r), shininess(s),
...
Vector3f Material::Shade(const Ray &ray, const Hit &hit,
  ...
  Vector3f n = hit.getNormal().normalized(); // Diffuse Shading
  if (noise.valid()) {
...
void Material::loadTexture(const char *filename) { t.load(filename); }

float Material::getRefractionIndex() { return refractionIndex; }

Vector3f Material::getDiffuseColor() { return diffuseColor; }

Vector3f Material::getSpecularColor() { return specularColor; }

void Material::setNoise(const Noise &n) { noise = n; }
